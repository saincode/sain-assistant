...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[#121212] text-white relative overflow-hidden">
      ...
      {/* Header */}
      <div className="flex items-center justify-between px-3 sm:px-6 py-3 border-b border-purple-800/30 bg-gradient-to-r from-[#13151a] to-[#1A1C23] relative gap-2">
        {/* Left: Logo and Name */}
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <svg width="32" height="32" className="sm:w-10 sm:h-10" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            {/* Brain shape */}
          ...
          <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-transparent bg-clip-text">sAIn</h1>
        </div>
        
        {/* Center: Tagline - Hidden on mobile */}
        <div className="hidden sm:block absolute left-1/2 transform -translate-x-1/2">
          <span className="text-purple-200/70 font-medium text-sm">Your Intelligent Document Assistant</span>
        </div>
  ...
  <div className="flex-1 flex flex-col">
        {messages.length === 0 && (
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-gradient-to-br from-purple-950/50 via-black/80 to-pink-950/50 backdrop-blur-lg border border-purple-800/30 shadow-2xl rounded-2xl px-4 sm:px-10 py-6 sm:py-8 flex flex-col items-center animate-fade-in-up w-11/12 sm:max-w-xl">
              <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4 bg-gradient-to-r from-purple-400 via-pink-400 to-purple-500 bg-clip-text text-transparent text-center">
                Your Personal AI Assistant
              </h2>
              <p className="text-gray-200 text-base sm:text-lg text-center mb-2 sm:mb-3 leading-relaxed">
        ...
        <div className="flex-1 overflow-y-auto">
          {messages.map((message, index) => (
            <div
              key={index}
              className={`px-2 sm:px-4 py-4 sm:py-6 mb-2 sm:mb-4 transition-all duration-500 animate-fade-in-up`}
              style={{ animationDelay: `${index * 60}ms` }}
            >
              <div className={`max-w-3xl mx-auto flex gap-2 sm:gap-4 p-3 sm:p-4 rounded-lg sm:rounded-xl ${
                message.role === 'assistant' 
                  ? 'bg-[#1e1f24] shadow-lg border border-purple-500/10' 
                  : 'bg-[#121212] border border-pink-500/10'
              }`}>
                <div
                  className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm shadow-lg transition-all duration-300 flex-shrink-0 ${
                    message.role === 'assistant'
        ...
        <div className="border-t border-gray-800 p-2 sm:p-4">
          <div className="max-w-3xl mx-auto space-y-2 sm:space-y-4">
            ...
            {messages.length > 0 && (
              <div className="flex justify-end px-1">
                <button
                  onClick={handleClear}
                  type="button"
                  className="group px-3 sm:px-4 py-2 sm:py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded-lg sm:rounded-xl transition-all duration-200 flex items-center gap-1 sm:gap-2 text-xs sm:text-sm border border-red-500/20 hover:border-red-500/30 shadow-lg hover:shadow-red-500/10"
                >
            ...
            <form onSubmit={handleSubmit} className="w-full px-1">
              <div className="flex items-center gap-2 sm:gap-3 bg-gradient-to-r from-blue-950/40 via-cyan-950/30 to-teal-950/40 rounded-full border border-cyan-500/20 p-1.5 sm:p-2 animate-fade-in-up shadow-xl hover:shadow-cyan-500/10 transition-all duration-300">
                <button
                  type="button"
                  onClick={handleFileUpload}
                  className="p-2 sm:p-2.5 hover:bg-cyan-500/20 rounded-full text-cyan-400 hover:text-cyan-300 transition-all duration-200 hover:scale-110 flex-shrink-0"
                  disabled={uploading}
                >
                ...
                <input
                  ...
                  className="flex-1 min-w-0 bg-transparent border-0 focus:ring-0 focus:outline-none text-cyan-100 placeholder-cyan-400/40 rounded-full px-2 sm:px-4 py-2 text-xs sm:text-sm"
                  disabled={loading}
                />
